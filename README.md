...
- Use python3.8, create virtual env and activate
- `$ mkdir .vscode && touch ./vscode/settings`
...
- This creates settings that apply to just this workspace

- Install the [ruff vscode extension](https://marketplace.visualstudio.com/items?itemName=charliermarsh.ruff) which will format, fix, and organize imports on-save

- `$ make install`
- `$ zsh all_test.sh`
  - Almost all tests should pass

# First project - 01_hello

- `$ cd 01_hello`
- `$ make test`
  - Should fail
- Follow `01_hello/README.md` and look at failing test case for instructions
- For example, the first failing test is that `./hello.py` exists
- `$ touch hello.py`
  - create the file
- `$ make test`
...
# Second project - 02_crowsnest

- When working on future projects included template by creating via
...
Then do `git clone` of _your_ repository to get a local copy.
